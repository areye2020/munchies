...
class CustomRestrictionsViewController: UIViewController, UITableViewDelegate,
    ...
    func tableView(_ tableView:UITableView, cellForRowAt indexPath:IndexPath) -> UITableViewCell {
        let cell:UITableViewCell = tableView
            .dequeueReusableCell(withIdentifier: restrictionCellIdentifier, for: indexPath)
    ...
    func tableView(_ tableView:UITableView, didSelectRowAt indexPath:IndexPath) {
        ...
        let submitAction:UIAlertAction = UIAlertAction(title: "submit",
            style: UIAlertAction.Style.default, handler: editRestriction)
        
        let alert:UIAlertController = UIAlertController(title: editTitle, message: editMessage,
    ...
    func addRestriction(alert:UIAlertAction) {
        do {
            var newRestriction:String =
                try removeExtraWhitespace(string: alertTextField.text ?? "")
            if newRestriction != "" {
    ...
    func createErrorAlert(message:String) {
        let cancelAction:UIAlertAction = UIAlertAction(title: "cancel",
            style: UIAlertAction.Style.cancel)
        let alert:UIAlertController = UIAlertController(title: "Error", message: message,
            preferredStyle: UIAlertController.Style.alert)
        alert.addAction(cancelAction)
        present(alert, animated: true)
    }
    ...
    @IBAction func onPressAdd(_ sender:UIBarButtonItem) {
        let cancelAction:UIAlertAction = UIAlertAction(title: "cancel",
            style: UIAlertAction.Style.cancel)
        let submitAction:UIAlertAction = UIAlertAction(title: "submit",
            style: UIAlertAction.Style.default, handler: addRestriction)
        
        let alert:UIAlertController = UIAlertController(title: addTitle, message: addMessage,
            preferredStyle: UIAlertController.Style.alert)
        alert.addTextField(configurationHandler: {(textField) in self.alertTextField = textField})
        alert.addAction(cancelAction)
        alert.addAction(submitAction)
